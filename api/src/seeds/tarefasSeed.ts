...
export default async function tarefasSeed(quantidade: number): Promise<void> {
    const tarefasCriadas: TarefasCriada[] = [];

    const usuarios = await Usuario.find().lean();
    const boards = await Board.find().lean();

    // Lista de status possíveis
    const statusPossiveis = ["Open", "Fazendo", "Feito", "Closed"];
    ...
    for (let i = 0; i < quantidade; i++) {
        ...
        const data_final = new Date(
            ...
            faker.date.future().getMonth(),
            faker.date.future().getDate()
        );
