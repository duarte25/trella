...
export default class BoardValidation {
    static async listarIDBoardValidate(req, res, next) {
        let val = new Validator(req.params);
        await val.validate("id", v.required(), v.mongooseID(), v.toMongooseObj({ model: Board, query: { _id: req.params.id } }));
        if (val.anyErrors())
            return sendError(res, 404, val.getErrors());
    ...
    static async CriarBoardValidate(req, res, next) {
    ...
    static async AlterarBoardValidate(req, res, next) {
        let val = new Validator(req.params);
        // Validar id
        await val.validate("id", v.required(), v.mongooseID(), v.toMongooseObj({ model: Board, query: { _id: req.params.id } }));
    ...
    static async deleteBoardValidate(req, res, next) {
        let val = new Validator(req.params);
        await val.validate("id", v.required(), v.mongooseID(), v.toMongooseObj({ model: Board, query: { _id: req.params.id } }));
        if (val.anyErrors())
            return sendError(res, 404, val.getErrors());
