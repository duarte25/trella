...
export default class TarefaValidation {
    static async CriarTarefaValidate(req, res, next) {
    ...
    static async AlterarTarefaValidate(req, res, next) {
        let val = new Validator(req.params);
        // Validar id
        await val.validate("id", v.required(), v.mongooseID(), v.toMongooseObj({ model: Tarefas, query: { _id: req.params.id } }));
    ...
    static async deleteTarefaValidate(req, res, next) {
        let val = new Validator(req.params);
        await val.validate("id", v.required(), v.mongooseID(), v.toMongooseObj({ model: Tarefas, query: { _id: req.params.id } }));
        if (val.anyErrors())
            return sendError(res, 404, val.getErrors());
